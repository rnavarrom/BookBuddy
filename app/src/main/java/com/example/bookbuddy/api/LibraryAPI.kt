...
interface LibraryAPI {
    ...
    */
    ...
    suspend fun getLibraries(@Path("name") libraryName: String, @Path("search") search: Boolean, @Path("position") position: Int): Response<List<Library>>

    @GET("/api/library/{library_id}/{position}")
    suspend fun getLibraryBooks(@Path("library_id") libraryId: Int, @Path("position") position: Int): Response<List<Book>>

    @POST("/api/library/{name}/{lat}/{lon}/{zip}")
    suspend fun insertLibrary(@Path("name") libraryName: String, @Path("lat") latitude: Double
                              , @Path("lon") longitude: Double, @Path("zip") zip: String): Response<Boolean>

    @PUT("/api/library/{id}/{name}/{lat}/{lon}/{zip}")
    suspend fun updateLibrary(@Path("id") libraryId: Int, @Path("name") libraryName: String
                              , @Path("lat") latitude: Double, @Path("lon") longitude: Double, @Path("zip") zip: String): Response<Boolean>
