 ...
 */
    ...
    suspend fun getUserImage(userId: Int): Response<ResponseBody> {
        val call = getRetrofit().create(ImageAPI::class.java).getUserImage(userId)
        return call
    }

    suspend fun uploadImageToAPI(isCover: Boolean,image: MultipartBody.Part): Response<ResponseBody> {
        val call = getRetrofit().create(ImageAPI::class.java).uploadImage(isCover, image)
        return call
    }
    /*
